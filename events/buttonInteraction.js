...
module.exports = {
    ...
    async execute(interaction) {
        ...
        **Status**\n\
        ...
        > ${lang.E.reply} \`${adSet}\``
        ...
        } else if (interaction.customId === "interview_settings_button") {
            // Databases
            ...
            **Status**\n\
            ...
            // Buttons
            let enable = new ButtonBuilder()
                .setCustomId("enable_interview_button")
                .setLabel("Enable")
                .setStyle(ButtonStyle.Success);

            let disable = new ButtonBuilder()
                .setCustomId("disable_interview_button")
                .setLabel("Disable")
                .setStyle(ButtonStyle.Danger);

            if (interviewSettings.enabled === true) {
                enable.setDisabled(true);
                disable.setDisabled(false);
            } else {
                enable.setDisabled(false);
            ...
            // Response
            return interaction.reply({
                embeds: [interviewPage],
                components: [row],
                ephemeral: true,
            });
        } else if (interaction.customId === "enable_interview_button") {
            // Get DB
            const [interviewSettings, created] =
                await InterviewSettings.findOrCreate({
                    where: { guildId: interaction.guild.id },
            ...
            if (interviewSettings.enabled === true) {
                ...
                await interviewSettings.update({ enabled: true });

                return interaction.reply({
                    content: `${lang.E.greenTick} The interview module has been enabled!`,
                    ephemeral: true,
                });
            }
        } else if (interaction.customId === "disable_interview_button") {
            // Get DB
            const [interviewSettings, created] =
                await InterviewSettings.findOrCreate({
                    where: { guildId: interaction.guild.id },
            ...
            if (networkSettings.enabled === true) {
                ...
                return interaction.reply({
                    content: `${lang.E.redCross} The interview module has been disabled!`,
                    ephemeral: true,
                });
            } else {
                return interaction.reply({
